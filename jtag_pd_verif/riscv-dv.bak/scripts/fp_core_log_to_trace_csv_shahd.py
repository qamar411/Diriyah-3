...
# # Regex for register values (operands)
# RS_RE = re.compile(r"(x(?P<rs>[1-9]\d*):0x(?P<rs_val>[0-9a-f]+))")
# FPRS_RE = re.compile(r"(f(?P<fprs>[0-9]+):0x(?P<fprs_val>[0-9a-f]+))")
...
def _process_core_sim_log_fd(log_fd, csv_fd, full_trace=True):
    ...
    for line in log_fd:
        ...
        # # Match GPR reads (rs: source registers) // to be continued ...
        # rs_matches = RS_RE.finditer(line)
        # for rs in rs_matches:
        #     if trace_entry is not None:
        #         if not hasattr(trace_entry, 'gpr_read') or trace_entry.gpr_read is None:
        #             trace_entry.gpr_read = []
        #         trace_entry.gpr_read.append('{}:{}'
        #                                 .format(gpr_to_abi("x{}".format(rs.group("rs"))),
        #                                         rs.group("rs_val")))

        # # Match FPR reads (rs: source registers)
        # fprs_matches = FPRS_RE.finditer(line)
        # for fprs in fprs_matches:
        #     if trace_entry is not None:
        #         if not hasattr(trace_entry, 'gpr_read') or trace_entry.gpr_read is None:
        #             trace_entry.gpr_read = []
        #         trace_entry.gpr_read.append('{}:{}'
        #                                 .format(gpr_to_abi("f{}".format(fprs.group("fprs"))),
        #                                         fprs.group("fprs_val")))
