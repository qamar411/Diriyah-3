...
def process_core_sim_log(core_log, csv, full_trace=1):
    ...
    Extract instruction and affected register information from core simulation
    ...
    try:
        with open(core_log, "r") as log_fd, open(csv, "w") as csv_fd:
            count = _process_core_sim_log_fd(log_fd, csv_fd,
...
def convert_operands_to_abi(operand_str):
    ...
    At this stage in the conversion of the core log to CSV format, the operand
...
def check_core_uvm_log(uvm_log, core_name, test_name, report, write=True):
    """Process core UVM simulation log.
