...
EDGE_FF = re.compile(
    r"(core\s+\d+:\s+)?(?P<pri>\d)\s+0x(?P<addr>[a-f0-9]+?)\s+" \
    r"\((?P<bin>.*?)\)" \
    r"(?:\s+(?P<csr1>\S+)\s+0x(?P<csr1_val>[a-f0-9]+))?" \
    r"\s+(?P<reg>[xf]\s*\d*?)\s+0x(?P<val>[a-f0-9]+)" \
    r"(?:\s+(?P<csr2>\S+)\s+0x(?P<csr2_val>[a-f0-9]+))?"\
    )
...
def read_spike_trace(path, full_trace):
    ...
    with open(path, 'r') as handle:
        for line in handle:
            ...
            ff_match = EDGE_FF.match(line)
            if ff_match and not commit_match:
                groups = ff_match.groupdict()
                instr.gpr.append(gpr_to_abi(groups["reg"].replace(' ', '')) +
                                    ":" + groups["val"])
